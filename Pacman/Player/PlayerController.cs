    ...
    public class PlayerController
    {
        ...
        public void movePlayer()
        {
            ...
            if (this.model.IsDead) //death scene
            {
                ...
                this.view.pictureBox1.Image = null; //clean up screen
        ...
        public void notify(bool isDead)
        {
            ...
            {
                if (observers[i] is EnemyController)
                {
                    //hide all enemies
                    observers[i].Model.Counter = 0;
                    observers[i].Model.X = 18;
                    observers[i].Model.Y = 16;
                    observers[i].View.Left = -32;
                    observers[i].View.Top = -32;
                }
            }
        }
        ...
        protected void notifyObservers()
        {
            ...
            for (int i=0; i < observers.Count; i++)
            {
                if (!(observers[i] is WorldController) && !(observers[i] is LivesController))
                {
                    observers[i].notify(this.model.X, this.model.Y, this.view.Left, this.view.Top); //gives coordinates of player to observers
                }
            }
        ...
        protected void notifyLives()
        {
            ...
            for (int i = 0; i < observers.Count; i++)
            {
                if (observers[i] is LivesController)
                {
                    observers[i].notify(this.model.IsDead);
                }
            }
            /* foreach (dynamic observer in this.observers)
            {
        ...
        protected void notifyWorld()
        {
            ...
            for (int i = 0; i < observers.Count; i++)
            {
                if (observers[i] is WorldController)
                {
                    observers[i].notify(true);
                }
            }
                /*foreach (dynamic observer in this.observers)
                {
